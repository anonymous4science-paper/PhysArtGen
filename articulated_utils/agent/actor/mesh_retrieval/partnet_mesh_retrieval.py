...
import logging
from articulated_utils.utils.clip_utils import (
...
from articulated_utils.utils.mesh_utils import add_mesh
...
def add_mesh_to_out_folder_no_scale(task: str, box_layout: List[Dict], meshes: Dict, task_out: str, overwrite: bool = False) -> List[Dict]:
    """
    Add mesh to output folder and update box layout without scaling.
    ...
    List[Dict]: Updated box layout.
    """
    from articulated_utils.utils.mesh_utils import add_mesh_no_scale
    ...
    for box_spec in box_layout:
        ...
        vertices, faces = add_mesh_no_scale(box_spec, mesh_file)
        if not os.path.exists(mesh_path) or overwrite:
            save_mesh_to_obj(vertices, faces, mesh_path)
