...
from articulated_utils.utils.metric import average_link_difference
...
class LinkPlacementActor(Agent):
    ...
    def _get_code_example(self):
        if self.cfg.modality == "text":
            ...
            example = 'articulated_utils/examples/link_placement_desc_examples.py'
        else:
            # Use the off-centered meshes from PartNet-Mobility so the placement needs not be so precise
            example = 'articulated_utils/examples/link_placement_examples.py'
