from articulated_utils.utils.cotracker_utils import make_cotracker
import glob
import os
from articulated_utils.preprocess.preprocess_utils import mask_urdf
from articulated_utils.agent.actor.mesh_retrieval.partnet_mesh_retrieval import make_links_from_json
from articulated_utils.api.odio_urdf import (
    compile_python_to_urdf,
    save_joint_states,
    process_urdf,
)
from articulated_utils.preprocess.preprocess_partnet import (
    get_urdf_file,
    render_object,
)
...
from articulated_utils.utils.prompt_utils import (
    remove_lines_containing,
    extract_code_from_string,
    get_n_examples_from_python_code,
)
from articulated_utils.utils.viz import get_frames_from_video, convert_mp4_to_gif
from articulated_utils.agent.multimodal_incontext_agent import InContextExampleModel
from typing import Optional
from articulated_utils.utils.metric import compute_joint_diff
from articulated_utils.utils.partnet_utils import get_joint_semantic
...
runs without any errors. DO NOT MODIFY the link placement code. Make sure to use the same link names as provided in the link placement code. Make sure to include the import statement `from articulated_utils.api.odio_urdf import *`.
...
- Only include this import statement `from articulated_utils.api.odio_urdf import *`. Remove any other import statements.
...
class JointPredictionActor(Agent):
    ...
    def _get_code_example(self):
        if self.cfg.joint_actor.targetted_affordance:
            example = "articulated_utils/examples/joint_examples_targetted.py"
        else:
            example = "articulated_utils/examples/joint_examples_all.py"
    ...
    def _make_image_prompt_parts(self, image_path):
        if "mp4" in image_path:
            ...
            if not frames:
                print(f"警告: 从视频 {image_path} 中未提取到帧")
                # 使用默认图像或跳过处理
                return None
            img = frames[0]
        else:
    ...
    def load_predicted_rendering(self, use_gif=False, joint_name=None):
        if self.cfg.modality == 'partnet' and self.cfg.joint_actor.targetted_semantic_joint:
            ...
            # 检查是否有视频文件
            if not videos:
                print(f"警告: 在目录 {self.cfg.out_dir} 中未找到视频文件")
                # 返回None或创建一个占位符，避免list index out of range错误
                return None
