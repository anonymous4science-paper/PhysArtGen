...
class PybulletRenderer:
    """简化的PyBullet渲染器，专用于critic-verifier系统"""
    ...
    def _init_pybullet(self):
        """初始化PyBullet"""
        try:
            self.client = connect_pybullet(headless=True)
            return True
        except Exception as e:
            print(f"初始化PyBullet失败: {e}")
            return False
    ...
    def render_urdf(self, urdf_path, output_path, camera_pos=[2, 0, 1], camera_target=[0, 0, 0.5]):
        ...
        try:
            ...
            with HideOutput():
                ...
                try:
                    os.chdir(urdf_dir)
                    self.robot_id = p.loadURDF(urdf_filename)
                finally:
            ...
            projection_matrix = p.computeProjectionMatrixFOV(
                fov=35,
                aspect=float(self.resolution[0]) / self.resolution[1],
                nearVal=0.1,
                farVal=100
            )
            ...
            cv2.imwrite(output_path, rgb_array)
