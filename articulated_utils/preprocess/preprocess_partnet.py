...
import os
from articulated_utils.utils.parallel_utils import process_tasks
from articulated_utils.utils.partnet_utils import get_obj_type
...
from articulated_utils.preprocess.preprocess_utils import (
    mask_urdf,
)
from typing import Optional
from articulated_utils.agent.actor.mesh_retrieval.partnet_mesh_annotator import combine_meshes
from articulated_utils.api.odio_urdf import process_urdf
import logging
...
def render_object(urdf_file: str, gpu_id: str, simulator_cfg: DictConfig, render_mode: str = "move"):
    ...
    command = config_to_command(
        simulator_cfg,
        "articulated_utils/physics/sapien_simulate.py",
    )
    env = os.environ.copy()
...
def render_parts(obj_id: str, gpu_id: str, cfg: DictConfig):
    ...
    command = make_cmd(
        script_path="articulated_utils/agent/actor/mesh_retrieval/partnet_mesh_annotator.py",
        cmd_args=[obj_dir]
    )
    env = os.environ.copy()
    env["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
